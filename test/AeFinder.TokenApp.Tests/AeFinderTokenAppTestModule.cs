...
using AeFinder.TokenApp.Orleans.TestBase;
...
[DependsOn(
    typeof(AeFinderTokenAppOrleansTestBaseModule),
    typeof(TokenAppModule),
    typeof(AbpAutoMapperModule),
    typeof(AbpAutofacModule),
    typeof(AeFinderAppModule))]
public class AeFinderTokenAppTestModule : AbpModule
{
    public override void ConfigureServices(ServiceConfigurationContext context)
    {
        Configure<AbpAutoMapperOptions>(options => { options.AddMaps<AeFinderTokenAppTestModule>(); });
    ...
    private async Task CreateIndexAsync(IServiceProvider serviceProvider)
    {
        ...
        var types = GetTypesAssignableFrom<IAeFinderEntity>(typeof(TokenAppModule).Assembly);
